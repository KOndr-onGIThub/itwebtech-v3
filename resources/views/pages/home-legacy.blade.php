{{-- ===================================================
     PŮVODNÍ HOMEPAGE — zmrazená kopie pro srovnání (OND-306)

     ⚠️ DOČASNÉ LEŠENÍ. Tohle je kopie `pages/home.blade.php` ke dni
     23. 9. 2026, odklopená na `/puvodni-homepage`, aby šlo během
     přestavby domovské stránky (OND-305) porovnat starou a novou verzi
     vedle sebe. Texty čte z `lang/{cs,en,de}/home_legacy.php` — vlastní
     kopie, takže přepis `home.php` sem nedosáhne.

     NEUDRŽOVAT a nesdílet s novou homepage. Až Ondřej novou verzi
     odsouhlasí, smaže se jedním commitem: tenhle soubor, tři
     `home_legacy.php`, `PageController::homeLegacy()`, dvě routy
     `puvodni-homepage` a test `HomeLegacyPageTest`.

     Původní hlavička souboru následuje beze změny:
     ---------------------------------------------------
...
@section('title', __('home_legacy.meta.title'))
@section('description', __('home_legacy.meta.description'))
...
<section class="pd-hero" id="pd-hero-tilt">
    ...
    <div class="container-site">
        <div class="pd-hero__content">
            <p class="pd-eyebrow">{{ __('home_legacy.hero.page_mark_label') }} — {{ __('home_legacy.hero.upline') }}</p>

            <h1 class="pd-heading">{!! __('home_legacy.hero.heading_html') !!}</h1>

            <p class="pd-sub">{{ __('home_legacy.hero.subline') }}</p>
            ...
            <div class="pd-actions">
                <a href="#{{ __('home_legacy.anchors.poptavka') }}" class="pd-cta" data-analytics="hero_cta_primary_click">
                    {{ __('home_legacy.hero.cta_primary') }}
                    <x-icon.arrow-right class="w-4 h-4 shrink-0 pd-cta__arrow" />
                </a>
                <x-phone-cta class="pd-phone" :label="__('home_legacy.hero.phone_label')" />
            </div>
            <p class="pd-note">{{ __('home_legacy.hero.note') }}</p>
        </div>
    </div>
</section>
...
<section class="pd-section">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.portfolio.heading') }}</h2>
        </header>
        <p class="pd-intro">{{ __('home_legacy.portfolio.intro') }}</p>

        <div class="pd-cases">
            @foreach ($featuredHomeProjects as $i => $project)
                @php
                    $t = $project->translation();
                    $cardCopy = __('home_legacy.portfolio.cards.' . $project->slug);
                ...
                <article class="pd-case">
                    <a
                        ...
                        aria-label="{{ $clientLabel }} — {{ __('home_legacy.portfolio.detail_cta') }}"
                    ...
                    <div class="pd-case__body">
                        ...
                        <p class="pd-case__links">
                            <a
                                ...
                                data-analytics="project_card_click"
                                data-analytics-props='{"slug":"{{ $project->slug }}"}'
                            >{{ __('home_legacy.portfolio.detail_cta') }} &rarr;</a>
                            @if ($project->live_url)
                            <a
                                ...
                                target="_blank"
                                rel="noopener"
                                aria-label="{{ __('home_legacy.portfolio.live_aria', ['client' => $clientLabel]) }}"
                                data-analytics="showcase_site_click"
                                data-analytics-props='{"site":"{{ $project->slug }}"}'
                            >{{ __('home_legacy.portfolio.live_cta') }} &nearr;</a>
                            @endif
                        </p>
                    </div>
                </article>
        ...
        <p class="pd-more"><a href="{{ lroute('projects') }}" class="pd-more__link">{{ __('home_legacy.portfolio.cta') }}</a></p>
    </div>
</section>
@endif
...
{{-- ===================================================
     ...
     =================================================== --}}
<section class="pd-strip" aria-label="{{ __('home_legacy.social_proof.rating_aria') }}">
    <div class="container-site">
        <ul class="pd-strip__list">
            <li><strong>{{ __('home_legacy.social_proof.rating_value') }}</strong> {{ __('home_legacy.social_proof.reviews') }}</li>
            <li><strong>{{ __('home_legacy.social_proof.projects') }}</strong></li>
            <li><strong>{{ __('home_legacy.social_proof.experience') }}</strong></li>
            <li>{{ __('home_legacy.social_proof.response') }}</li>
            <li>{{ __('home_legacy.social_proof.award') }}</li>
        </ul>
    </div>
</section>
...
<section class="pd-section">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.problems.heading') }}</h2>
        </header>
        <p class="pd-lead">{{ __('home_legacy.problems.lead') }}</p>

        <p class="pd-avoid">{{ __('home_legacy.problems.transition_heading') }}</p>
        <p class="pd-avoid-sub">{{ __('home_legacy.problems.transition_text') }}</p>

        <div class="pd-issues">
            @foreach (__('home_legacy.problems.items') as $i => $item)
        ...
        <p class="pd-avoid pd-avoid--second">{{ __('home_legacy.problems.ai_heading') }}</p>
        <p class="pd-avoid-sub">{{ __('home_legacy.problems.ai_text') }}</p>
    </div>
</section>
...
<section class="pd-section">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.services.heading_primary') }}</h2>
        </header>
        ...
        <div class="pd-services">
            ...
            <article class="pd-service">
                ...
                <h3 class="pd-service__title">{{ __("home_legacy.services.primary.{$key}.title") }}</h3>
                <p class="pd-service__desc">{{ __("home_legacy.services.primary.{$key}.description") }}</p>
                <ul class="pd-service__bullets">
                    @foreach (__("home_legacy.services.primary.{$key}.bullets") as $bullet)
                    <li>{{ $bullet }}</li>
                    @endforeach
                </ul>
            </article>
        ...
        <p class="pd-services__secondary">
            {!! __('home_legacy.services.secondary_inline', [
                'pricing_link' => '<a href="' . lroute('price') . '">' . e(__('home_legacy.services.secondary_inline_pricing')) . '</a>',
                'contact_link' => '<a href="#' . __('home_legacy.anchors.poptavka') . '">' . e(__('home_legacy.services.secondary_inline_contact')) . '</a>',
            ]) !!}
        </p>
    </div>
</section>
...
<section class="pd-section" id="section-price" data-analytics-view="price_anchor_view">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.price_anchor.heading') }}</h2>
        </header>
        <p class="pd-intro">{{ __('home_legacy.price_anchor.intro') }}</p>

        <div class="pd-price">
            @foreach (__('home_legacy.price_anchor.items') as $item)
            <div class="pd-price__col {{ ($item['featured'] ?? false) ? 'pd-price__col--featured' : '' }}">
                <h3 class="pd-price__title">{{ $item['title'] }}@if ($item['featured'] ?? false) <em>{{ __('home_legacy.price_anchor.featured_label') }}</em>@endif</h3>
        ...
        <p class="pd-more">
            <a href="{{ lroute('price') }}" class="pd-more__link" data-analytics="price_anchor_cta_click">
                {{ __('home_legacy.price_anchor.cta') }}
            </a>
        </p>
    </div>
</section>
...
<section class="pd-section">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.why_me.heading') }}</h2>
        </header>

        <div class="pd-why">
            <div class="pd-why__media">
                <div class="pd-why__video">
                    <x-video-intro :ariaLabel="__('home_legacy.why_me.video_aria')" />
                </div>
                <p class="pd-why__bio">{{ __('home_legacy.why_me.bio') }}</p>
            </div>

            <div class="pd-why__grid">
                @foreach (__('home_legacy.why_me.advantages') as $i => $adv)
        ...
        <div class="pd-origin pd-origin--nested">
            <h3 class="pd-origin__title">{{ __('home_legacy.toyota.heading') }}</h3>
            <div class="pd-origin__text">
                <p>{{ __('home_legacy.toyota.text') }}</p>
                <p>{{ __('home_legacy.toyota.text_2') }}</p>
            </div>
            <blockquote class="pd-origin__quote">
                {{ __('home_legacy.toyota.quote_text') }}
                <footer>— {{ __('home_legacy.toyota.quote_author') }}</footer>
            </blockquote>
        </div>
    </div>
</section>
...
<section class="pd-section">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.craft.heading') }}</h2>
        </header>
        <p class="pd-intro">{{ __('home_legacy.craft.intro') }}</p>

        <div class="pd-hood">
            @foreach (__('home_legacy.craft.facts') as $fact)
        ...
        <p class="pd-hood__perf" id="pd-perf" hidden>
            {{ __('home_legacy.craft.perf_prefix') }}
            <strong id="pd-perf-value"></strong>
            {{ __('home_legacy.craft.perf_suffix') }}
        </p>
...
{{-- ===================================================
     ...
     =================================================== --}}
<section class="pd-section" id="{{ __('home_legacy.anchors.how_i_work') }}">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.how_i_work.heading') }}</h2>
        </header>

        <ol class="pd-steps">
            @foreach (__('home_legacy.how_i_work.steps') as $i => $step)
            <li class="pd-step">
        ...
        <div class="pd-steps__cta">
            <p class="pd-steps__cta-intro">{{ __('home_legacy.how_i_work.cta_intro') }}</p>
            <a href="#{{ __('home_legacy.anchors.poptavka') }}" class="pd-cta" data-analytics="final_cta_secondary_click">
                {{ __('home_legacy.how_i_work.cta_label') }}
                <x-icon.arrow-right class="w-4 h-4 shrink-0 pd-cta__arrow" />
            </a>
        </div>
    </div>
...
<section class="pd-section" id="section-testimonials" data-analytics-view="testimonial_view">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.testimonials.heading') }}</h2>
        </header>
        ...
        @if (filled(__('home_legacy.testimonials.note')))
        <p class="pd-testi__note">{{ __('home_legacy.testimonials.note') }}</p>
        @endif
...
{{-- ===================================================
     ...
     =================================================== --}}
@php
    $faqItems = __('home_legacy.faq.items');
@endphp
<section class="pd-section" id="faq">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.faq.heading') }}</h2>
        </header>
...
{{-- ===================================================
     ...
     =================================================== --}}
<section class="pd-section" id="{{ __('home_legacy.anchors.poptavka') }}">
    <div class="container-site">
        <header class="pd-head">
            <h2 class="pd-head__title">{{ __('home_legacy.inline_form.heading') }}</h2>
        </header>

        <div class="pd-form">
            <div class="pd-form__intro">
                <p class="pd-intro">{{ __('home_legacy.inline_form.description') }}</p>
                <blockquote class="pd-form__quote">
                    {{ __('home_legacy.inline_form.quote_text') }}
                    <footer>— {{ __('home_legacy.inline_form.quote_author') }}</footer>
                </blockquote>
            </div>
            ...
            <div class="pd-form__panel">
                @if (session('home_lead_success') || session('faq_lead_success'))
                    <div class="pd-alert pd-alert--success" role="status">
                        {{ __('home_legacy.inline_form.success') }}
                    </div>
                @endif
                ...
                >
                    ...
                    <div class="pd-form__grid">
                        <div class="pd-field">
                            <label for="pd-lead-name">{{ __('home_legacy.inline_form.name') }} <span aria-hidden="true">*</span></label>
                            <input
                                type="text"
                                id="pd-lead-name"
                                name="name"
                                value="{{ old('name') }}"
                                required
                                placeholder="{{ __('home_legacy.inline_form.placeholders.name') }}"
                                autocomplete="name"
                            >
                        ...
                        <div class="pd-field">
                            <label for="pd-lead-email">{{ __('home_legacy.inline_form.email') }} <span aria-hidden="true">*</span></label>
                            <input
                                type="email"
                                id="pd-lead-email"
                                name="email"
                                value="{{ old('email') }}"
                                required
                                placeholder="{{ __('home_legacy.inline_form.placeholders.email') }}"
                                autocomplete="email"
                            >
                        ...
                        <div class="pd-field pd-field--full">
                            <label for="pd-lead-phone">{{ __('home_legacy.inline_form.phone') }}</label>
                            <input
                                type="tel"
                                id="pd-lead-phone"
                                name="phone"
                                value="{{ old('phone') }}"
                                placeholder="{{ __('home_legacy.inline_form.placeholders.phone') }}"
                                autocomplete="tel"
                                aria-describedby="pd-lead-phone-hint"
                            >
                            {{-- OND-256/4: pošťouchnutí — proč číslo vyplnit, když je nepovinné. --}}
                            <p class="pd-field__hint" id="pd-lead-phone-hint">{{ __('home_legacy.inline_form.phone_hint') }}</p>
                            @error('phone') <p class="pd-field__error">{{ $message }}</p> @enderror
                        </div>

                        <div class="pd-field pd-field--full">
                            <label for="pd-lead-message">{{ __('home_legacy.inline_form.message') }} <span aria-hidden="true">*</span></label>
                            <textarea
                                id="pd-lead-message"
                                name="message"
                                rows="5"
                                required
                                placeholder="{{ __('home_legacy.inline_form.placeholders.message') }}"
                    ...
                    >
                        <span x-show="!submitting">{{ __('home_legacy.inline_form.submit') }}</span>
                        <span x-show="submitting" x-cloak>{{ __('home_legacy.inline_form.submitting') }}</span>
                    </button>

                    <p class="pd-form__note">{{ __('home_legacy.inline_form.note') }}</p>

                    <p class="pd-form__privacy">
                        {{ __('home_legacy.inline_form.privacy_prefix') }}<a href="{{ lroute('privacy') }}">{{ __('home_legacy.inline_form.privacy_link') }}</a>.
                    </p>
                </form>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('{{ __('home_legacy.anchors.poptavka') }}')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
        @endif
